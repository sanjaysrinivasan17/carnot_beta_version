...
export class DashboardMapComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      .then(data => {
        ...
        for (var i = 0; i < this.project_id_summary_alldata.length; i++) {
          ...
          if (this.str != '') {
            ...
            this.center.push({ "lat": this.lat, "long": this.long, "name": this.main_data_get_all[i]["name"], "category": this.main_data_get_all[i]['category'] })
    ...
    fetch(environment.api_name + "api/project/get_dashboard_data", { headers })
      .then(response => response.json())
      .then(data => {
        this.get_dashboard_data = data
        // this.Dashboard_data(this.get_dashboard_data)
      })
  }

  Dashboard_data(get_dashboard_data, category) {
    this.dashboard_total_key = Object.keys(get_dashboard_data['dashboard_total'])
    // alert(this.dashboard_total_key)
    this.dashboard_total_values = Object.values(get_dashboard_data['dashboard_total'])
    ...
    if (category == 'All' || category == this.categorywise_project[0]) {
      ...
      var thermography_project = this.processed_data_project_categorywise.filter(function (hero) {
        ...
        for (var m = 0; m < array.length; m++) {
          ...
          for (var n = 0; n < summary_keys.length; n++) {
            if (summary_keys[n] == 'Hotspot') {
              ...
              hotspot_val = hotspot_val + array[m]['summary_layers'][summary_keys[n]]['Count']
            } else if (summary_keys[n] == "Short Circuit") {
              var short_circuit_key = summary_keys[n]
              short_circuit = short_circuit + array[m]['summary_layers'][summary_keys[n]]['Count']
            } else if (summary_keys[n] == "Open Circuit") {
              var open_circuit_key = summary_keys[n]
              open_circuit = open_circuit + array[m]['summary_layers'][summary_keys[n]]['Count']
            } else if (summary_keys[n] == "Uniform Panel Heating" || summary_keys[n] == "Panel Failure") {
              var upf_or_pf_key = summary_keys[n]
              upf_or_pf = upf_or_pf + array[m]['summary_layers'][summary_keys[n]]['Count']
    ...
    this.Dashboard_graph(this.dashboard_total_key, this.dashboard_total_values, this.Total_project)
  ...
  map_location_marker(category) {
    ...
    var category_wise_color = { "vegetation": 'blue', 'thermography': 'green', 'topography': 'red', 'grading': 'yellow', 'due diligence': 'orange' }

    if (category == 'All' || category == this.categorywise_project[0]) {
      this.project_list = this.processed_data_project_categorywise_backup
    } else {
      this.project_list = this.processed_data_project_categorywise.filter(function (hero) {
    ...
    for (this.j = 0; this.j < this.project_list.length; this.j++) {
      ...
      var mapIcon = L.icon({
        ...
        iconUrl: 'http://maps.google.com/mapfiles/ms/icons/' + category_wise_color[this.project_list[this.j][0]["category"]] + '-dot.png',
      ...
      this.marker = L.marker([this.lat, this.long], { icon: mapIcon }).bindPopup(this.project_list[this.j][0]["name"])
