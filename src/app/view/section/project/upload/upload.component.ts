...
export class UploadComponent implements OnInit {
  ...
  onSelect_AdditionalFile(event: any) {
    // console.log();
    if (event.target.files.length > 0) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        fileReader.addEventListener('load', fileReaderEvent => {
          ...
          if (data) {
            ...
            var lat_hours = this.res['GPSLatitude'][0]["numerator"]
            var lat_min = (this.res['GPSLatitude'][1]["numerator"])
            var lat_secs = (this.res['GPSLatitude'][2]["numerator"])
            var lat_mins_secs = (lat_min / 60) + ((lat_secs / 10000) / 3600)
            var lat = lat_hours + lat_mins_secs
            var long_hours = this.res['GPSLongitude'][0]["numerator"]
            var long_min = (this.res['GPSLongitude'][1]["numerator"])
            var long_secs = (this.res['GPSLongitude'][2]["numerator"])
            var long_mins_secs = (long_min / 60) + ((long_secs / 10000) / 3600)
            var long = long_hours + long_mins_secs
            this.lat_long.push({
              "latitude": lat,
              "longitude": long
            })
  ...
  // Additional_file_data(event, lat_long) {
  ...
  show_additional_file_data(project_id: any, date: any) {
    ...
    var url = environment.api_name + 'api/project/get_thermal_assets/' + project_id + '/' + date
    fetch(url, { headers })
      .then(response => response.json())
      .then(datavalue => {
        var main_data = datavalue['data']
        // console.log(main_data)
        main_data.forEach((element: { [x: string]: string; }) => {
          // // console.log()
